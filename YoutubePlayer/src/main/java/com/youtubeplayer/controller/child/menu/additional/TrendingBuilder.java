...
public class TrendingBuilder {
    ...
    public TrendingBuilder(HBox parent) {
        this.parent = parent;
    }
    
    //START TRENDING CONTENT
    /**
     * Build grid trending content
     * @param list
     * @return content multiple rows
     */
    public GridPane buildTrending(List<Video> list){
        ...
        int row = 0;
        for (Video v : list) {
            grid.add(trendingButton(v), 0, row++);
        }
        return grid;
    }
    ...
    private JFXButton trendingButton(Video v){
        Image img = null;
        try {
