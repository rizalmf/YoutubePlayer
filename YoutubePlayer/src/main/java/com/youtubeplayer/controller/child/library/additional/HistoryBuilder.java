...
public class HistoryBuilder {
    ...
    public HistoryBuilder(HBox parent) {
        this.parent = parent;
    }
    
    //START HISTORY CONTENT
    /**
     * Build grid history content
     * @param list
     * @return content multiple rows
     */
    public GridPane buildHistory(List<Video> list){
        ...
        int row = 0;
        for (Video v : list) {
            grid.add(historyButton(v), 0, row++);
        }
        return grid;
    }
    ...
    private JFXButton historyButton(Video v){
        Image img = null;
        try {
