...
public class SearchBuilder {
    ...
    public SearchBuilder(HBox parent) {
        this.parent = parent;
    }
    
    //START SEARCH CONTENT
    /**
     * Build grid search content
     * @param list
     * @return content multiple rows
     */
    public GridPane buildSearch(List<Video> list){
    ...
    private JFXButton searchButton(Video v){
        Image img = null;
        try {
        ...
        Label lDuration;
        if (v.getLiveBroadcastContent().equals("live")) {
            FontAwesomeIconView view =  new FontAwesomeIconView(FontAwesomeIcon.CIRCLE);
            view.setFill(Paint.valueOf("red"));
            view.setSize("7");
            lDuration = new Label("live now", view);
        }else{
            lDuration = new Label(v.getPublishedAt());
        }
